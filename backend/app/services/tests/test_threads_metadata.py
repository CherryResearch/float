...
def _load_threads_service():
    ...
    app_pkg.__path__ = [str(REPO_ROOT / "backend" / "app")]
    sys.modules.setdefault("app", app_pkg)

    services_pkg = types.ModuleType("app.services")
    services_pkg.__path__ = [str(REPO_ROOT / "backend" / "app" / "services")]
    sys.modules["app.services"] = services_pkg

    utils_pkg = types.ModuleType("app.utils")
    utils_pkg.__path__ = [str(REPO_ROOT / "backend" / "app" / "utils")]
    sys.modules["app.utils"] = utils_pkg

    conversation_store = types.ModuleType("app.utils.conversation_store")
    conversation_store.CONV_DIR = REPO_ROOT / "data" / "conversations"
    ...
    def resolve_cluster_backend(cluster_backend, cluster_device):
        return {
            "backend": cluster_backend or "sklearn",
            "device": cluster_device or "cpu",
        }
    ...
    semantic.resolve_cluster_backend = resolve_cluster_backend
...
def test_read_summary_backfills_generated_timestamp_from_mtime(tmp_path):
    ...
    assert generated_at
    assert (
        summary.get("metadata", {}).get("ui_hints", {}).get("generated_at_utc")
        == generated_at
    )
